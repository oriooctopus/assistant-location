...
//
// GL_BAKED_HOST is the one secret value; every other build-time URL in the
// app (see Modules/Events, Modules/Todos) composes its own endpoint from this
// host plus its own port/path, so only a single value needs to be injected.
//
...
#define GL_BAKED_ENDPOINT ([NSString stringWithFormat:@"http://%@:8302/overland", GL_BAKED_HOST])
